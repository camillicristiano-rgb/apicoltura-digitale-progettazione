Materiali utilizzati e loro funzione
1. Router STRONG 4G LTE 300 mini (46,95 € x 2)
Funzione:
I router permettono la connessione a Internet dell’arnia, rendendo possibile la trasmissione dei dati (temperatura, umidità, peso, stato generale) verso l’apicoltore, anche a chilometri di distanza.

Perché ne servono tre:

Maggiore stabilità della rete

Possibilità di coprire aree isolate o con segnale debole

Ridondanza: se uno smette di funzionare, l’altro garantisce la continuità del servizio

Vantaggio per l’apicoltore:
Controllo dell’arnia in tempo reale senza doversi recare fisicamente sul posto.

2. Kit pannello solare + alternatore (56,99 €)
Funzione:
Fornisce energia elettrica autonoma a tutto il sistema.

Perché è fondamentale:

Le arnie spesso si trovano in zone isolate

Non è necessario collegarsi alla rete elettrica

Energia rinnovabile e gratuita dopo l’installazione visto che il cliente ce l’aveva chiesto

Vantaggio per l’apicoltore:
Sistema sempre attivo, costi energetici pari a zero e rispetto per l’ambiente.

3. Batterie 12V (9,04 € x 2)
Funzione:
Accumulo dell’energia prodotta dal pannello solare.

Perché ne servono tre:

Garantire funzionamento continuo anche di notte o in caso di maltempo

Maggiore affidabilità e durata del sistema

Vantaggio per l’apicoltore:
Nessuna interruzione nel monitoraggio dell’arnia.

4. Custodie in plastica (43,80 € x 3)
Funzione:
Proteggono i componenti elettronici (router, batterie, centraline).

Perché sono necessarie:

Protezione da pioggia, umidità, polvere e insetti che possono entrare all’interno

Maggiore sicurezza e durata dei dispositivi

Ordine e facilità di manutenzione

Vantaggio per l’apicoltore:
Meno guasti, meno spese future e maggiore affidabilità nel tempo.

5. Fascette in plastica (1,99 €)
Funzione:
Fissaggio ordinato e sicuro dei cavi e dei componenti.

Perché sono importanti:

Evitano danni ai cavi

Rendono il sistema più pulito e professionale

Facilitano interventi di manutenzione

Vantaggio per l’apicoltore:
Installazione stabile e duratura con costi minimi.

Perché l’apicoltore dovrebbe investire 440,68 €
Questo investimento iniziale permette di ottenere:

Monitoraggio continuo dell’arnia

Riduzione delle visite fisiche, risparmiando tempo,carburante, riducendo l’impatto traumatizzante sulle api

Prevenzione di problemi (surriscaldamento, furti)

Sistema autonomo ed ecologico

Risparmio economico nel lungo periodo
